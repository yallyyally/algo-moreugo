...
public class Solution {
	...
	static char[][] map;
	static String curDir; // 현재 방향
	static int Width, Height;
	static int[] di = { -1, 1, 0, 0 };
	static int[] dj = { 0, 0, -1, 1 };
	static char[] shape = {'^','v','<','>'};

	public static void updateMap(String keyInput) {
		int ni, nj;
		ni = posX;
		nj = posY;
		if (keyInput.equals("S")) { // 포탄 발사
			do {
			ni += di[Direction.valueOf(curDir).ordinal()];
			nj += dj[Direction.valueOf(curDir).ordinal()];
			...
			// 이동못해도 방향 변경은 ㄱㄴ
			curDir = keyInput;
			map[posX][posY] = shape[Direction.valueOf(curDir).ordinal()];
			ni += di[Direction.valueOf(curDir).ordinal()];
			nj += dj[Direction.valueOf(curDir).ordinal()];
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for (int t = 1; t <= T; t++) {
			...
			for (int i = 0; i < keyLength; i++) {
				updateMap(Character.toString(inputLine.charAt(i)));
			}
