...
class Solution {
    ...
    public void putIntoCourse(StringBuilder selectedDishes){
        boolean found = false;
//        System.out.println("뽑힌 조합 "+selectedDishes);
        //HashMap 쓰려고 했는데 그럼 끝에 정렬을 못해서.. 순차탐색
        int len = selectedDishes.length(); //그 조합의 길이
        for(int i=0; i<courseDishes.size(); i++){
  //          System.out.println("현재 "+courseDishes.get(i));
          ...
          //      System.out.println(i+"번째에 추가 "+courseDishes.get(i).cnt+"개 누적");
        ...
        if(!found){
          ...
          //  System.out.println("없어서 추가함 "+courseDishes.get(courseDishes.size()-1));
        }
    }
    ...
    public String[] solution(String[] orders, int[] course) {
        ...
        for(Dishes d : courseDishes){
         //   System.out.println("현재 요리 "+d.dishComb);
            if(d.cnt >= 2 && d.cnt == maxCnt.get(d.dishComb.length()) ){
         //       System.out.println(d.dishComb.toString()+"추가 ");
